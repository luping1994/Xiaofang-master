...
public class ClusterOverlay implements
    ...
    private List<ClusterItem> companyClusterItems;
...
//默认最多会缓存80张图片作为聚合显示元素图片,根据自己显示需求和app使用内存情况,可以修改数量
        ...
        if (clusterItems != null) {
            companyClusterItems = clusterItems;
        } else {
            companyClusterItems = new ArrayList<ClusterItem>();
        }
        mContext = context;
    ...
    public boolean onMarkerClick(Marker arg0) {
        ...
        if (cluster != null) {
            mClusterClickListener.onClusterItemClick(arg0, cluster.getClusterItems());
            return true;
        }
        return false;
    }
    ...
    private void calculateClusters() {
        ...
        for (ClusterItem clusterItem : companyClusterItems) {
            if (mIsCanceled) {
                return;
            }
